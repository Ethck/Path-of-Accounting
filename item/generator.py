...
class Item(BaseItem):
    ...
    def get_json(self):
        ...
        json["query"]["filters"]["misc_filters"]["filters"]["synthesised_item"] = { "option": self.synthesised }
        json["query"]["filters"]["misc_filters"]["filters"]["corrupted"] = { "option": self.corrupted }
        json["query"]["filters"]["misc_filters"]["filters"]["mirrored"] = { "option": self.mirrored }
        json["query"]["filters"]["misc_filters"]["filters"]["veiled"] = { "option": self.veiled }
        json["query"]["filters"]["misc_filters"]["filters"]["identified"] = { "option": self.identified }
        ...
        if self.sockets:
            ...
            if sockets == 6:
                json["query"]["filters"]["socket_filters"]["filters"]["sockets"] = {"min": 6}
    ...
    def create_pseudo_mods(self):
        ...
        if total_ele_resists > 0:
            mod = (get_item_modifiers_by_id("pseudo.pseudo_total_elemental_resistance"), total_ele_resists)
            self.mods.append(mod)
            print("[o] Combining the "+ Fore.CYAN+ f"elemental resistance"+ Fore.RESET+ " mods from the list into a pseudo-parameter")
            print("[+] Pseudo-mod "+ Fore.GREEN+ f"+{total_ele_resists}% total Elemental Resistance (pseudo)"+ Fore.RESET)

        if total_chaos_resist > 0:
            mod = (get_item_modifiers_by_id("pseudo.pseudo_total_chaos_resistance"), total_chaos_resist)
            self.mods.append(mod)
            print("[o] Combining the "+ Fore.CYAN+ f"chaos resistance"+ Fore.RESET+ " mods from the list into a pseudo-parameter")
            print("[+] Pseudo-mod "+ Fore.GREEN+ f"+{total_chaos_resist}% total Chaos Resistance (pseudo)")

        if total_life > 0:
            mod = (get_item_modifiers_by_id("pseudo.pseudo_total_life"), total_life)
            self.mods.append(mod)
            print("[o] Combining the "+ Fore.CYAN+ f"maximum life"+ Fore.RESET+ " mods from the list into a pseudo-parameter")
            print("[+] Pseudo-mod "+ Fore.GREEN+ f"+{total_life} to maximum Life (pseudo)"+ Fore.RESET)
    ...
    def get_json(self):
        ...
        json["query"]["filters"]["map_filters"] = {
            "filters": {
                ...
                "map_blighted": {
                    "option": self.base.startswith("Blighted")
                }
            }
        }
        if self.rarity == 'unique':
...
def parse_mod(mod_text: str, mod_values, category = ''):
    ...
    if category == 'weapon':
        if 'Adds # to #' in mod_text and not 'to Spells' in mod_text or 'to Accuracy Rating' in mod_text:
            mod = get_item_modifiers_by_text((mod_text + ' (Local)', mod_type))
        elif 'increased Attack Speed' in mod_text:
            mod = get_item_modifiers_by_text(('+#% total Attack Speed', ItemModifierType.PSEUDO))
        elif 'increased Physical Damage' in mod_text:
            mod = get_item_modifiers_by_text(('#% total increased Physical Damage', ItemModifierType.PSEUDO))
        elif 'increased Damage with Poison' in mod_text and not 'chance to Poison on Hit' in prev_mod:
            mod = get_item_modifiers_by_text((mod_text + ' (Local)', mod_type))


    if category == 'armour':
        if 'Armour' in mod_text or 'Evasion' in mod_text or 'Energy Shield' in mod_text:
            mod = get_item_modifiers_by_text((mod_text + ' (Local)', mod_type))
    ...
    for i in range(2,len(regions)):
        for line in regions[i]:
            ...
            if line.startswith('Area is influenced by'):
                ...
                mod = get_item_modifiers_by_text(('Area is influenced by #', ItemModifierType.IMPLICIT))
            elif line.startswith('Map is occupied by'):
                mod_value = mod_text.replace('Map is occupied by ', '')
                mod = get_item_modifiers_by_text(('Map is occupied by #', ItemModifierType.IMPLICIT))
        ...
        if first_line.startswith('Requirements'):
            ...
            for line in regions[i]:
                if 'Veiled Prefix' in line or 'Veiled Suffix' in line:
                    ...
                    if 'if you have at least' in line or 'inflicted with this Weapon to deal' in line:
                        mod_values = re.search(r'[+-]?\d+\.?\d?\d?', line).group(0)
                        numberIndex = re.search(r'[+-]?\d+\.?\d?\d?', line).start()
                        textIndex = line.find('if you have at least')
                        if textIndex == -1:
                            textIndex = line.find('inflicted with this Weapon to deal')
                        if numberIndex < textIndex:
                            mod_text = re.sub(r'[+-]?\d+\.?\d?\d?', '#', line, 1)
                        else:
                            mod_text = line
                    else:
                        mod_values = re.findall(r'[+-]?\d+\.?\d?\d?', line)
                        if len(mod_values) > 1:
                            try:
                                mod_values = ((float(mod_values[0])) + float(mod_values[1])) / 2
                            except ValueError:
                                mod_values = None
                        else:
                            if mod_values:
